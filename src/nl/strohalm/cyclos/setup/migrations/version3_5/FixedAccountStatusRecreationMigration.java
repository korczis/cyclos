...
public class FixedAccountStatusRecreationMigration implements UntraceableMigration {
    ...
    public void execute(final JDBCWrapper jdbc) throws SQLException {
        ...
        final String transferQuery = "select id, nullif(process_date, date) as date, amount, status, from_account_id, to_account_id, parent_id, account_fee_log_id, chargeback_of_id from transfers where status in ('O', 'P') and (from_account_id = ? or to_account_id = ?) order by nullif(process_date, date), id";
